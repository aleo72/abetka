...
object Abetka {
  ...
  def toLatin(baseText: String): String = {
    ...
    baseText.foreach { ch =>
      ...
      if (baseIndex >= 0 && baseIndex <= 55) {
        targetText.append(sL.charAt(baseIndex))
      } else if (baseIndex > 55) {
    ...
    baseText.foreach {
      ch =>
        ...
        if(baseIndex > 53) {
          ...
          val targetChar =
            if (baseIndex > -1) sK.charAt(baseIndex)
            else ch
          if(f == 0)
            targetText.append(targetChar)
          else if(f == 1 && baseIndex2 == -1)
            targetText.append('й').append(targetChar)
          else if(f == 1)
            targetText.append(sK.charAt(baseIndex2 + 56))
          else if (f == 2)
            targetText.append(sL.charAt(54)).append(sK.charAt(baseIndex2 + 56))
          f = 0
        }
    }
    if (f == 1)
